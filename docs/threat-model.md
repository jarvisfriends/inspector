...
Scope: inspector (a drop-in debug/inspector overlay for Bubble Tea v2 applications) and its released artifacts (the
`inspector` demo binary).
...
- Privilege escalation: the software runs entirely with the invoking user's privileges and opens no
  network listeners beyond the loopback-only, user-initiated pprof endpoint.
