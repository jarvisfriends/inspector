...
Every release of inspector ships the `inspector` demo binary, a `checksums.txt` covering all archives, an SPDX SBOM per
archive, and a Sigstore bundle `checksums.txt.sigstore.json` produced by keyless cosign signing in the
release workflow. Releases also carry `demo.gif`, rendered from `cmd/inspector/demo.tape` and attached via
`release.extra_files`; it is a documentation asset and is deliberately outside the signed checksum set.
...
cosign verify-blob \
  ...
  --certificate-identity "https://github.com/jarvisfriends/inspector/.github/workflows/release.yml@refs/tags/<TAG>" \
...
```bash
grype sbom:./inspector_<...>.sbom.json
```
